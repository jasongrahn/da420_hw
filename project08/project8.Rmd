...
The chi-square is `r with(mylogit, null.deviance - deviance)` and has `r with(mylogit, df.null - df.residual)` degrees of freedom. This provides a p-value of `r with(mylogit, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE))`. This is obviously much less than any measurable test for fit. The interpretation is that the model fits better than any random selection. 
