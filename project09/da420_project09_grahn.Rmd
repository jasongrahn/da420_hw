...
library(e1071) # used for SVM functions
```
...
```{r create the train vs test data separation, message=FALSE}
set.seed(666)
...
The table shows the KNN output against the known labels for the iris data. I see I mispredicted 1 _virginica_ as a _versacolor_. Otherwise we have accurate predictions across the board.
...
Nice! We have `r knn.accuracy.rate[2,3]` percent accuracy with our $k$nn!
...
That _is_ a surprise! We have `r svm.accuracy.rate$rate[1]` percent accuracy with our SMV!
...
The difference in accuracy between the two methods `SVM` and `KNN` are fairly close. SVM came in at `r svm.ac` percent accuracy, while KNN came in at `r knn.ac` percent. This is a difference of `r svm.ac - knn.ac`.